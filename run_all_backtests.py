#!/usr/bin/env python3
"""
run_all_backtests.py — NEXUS Backtester Orchestrator V1.0
==========================================================
Entry point for nexus-analyzer Railway worker (genuine-reverence).
Runs all three backtests in sequence every Sunday 11pm UTC.

Railway cron: 0 23 * * 0

Order:
  1. Berserker (nexus_analyzer_1min_railway.py V3.0) — ~35-45 min
  2. Phase4 (phase4_backtester.py V1.0)             — ~45-60 min
...
def main():
    ...
    print(f"[ORCHESTRATOR] NEXUS Backtester Orchestrator V1.0", flush=True)
    print(f"[ORCHESTRATOR] Start: {now.strftime('%Y-%m-%d %H:%M:%S CDT')}", flush=True)

    send_alert(
        f"🚀 NEXUS BACKTESTER SUITE STARTING\n"
        f"3 systems: Berserker → Phase4 → Crypto\n"
        f"ETA: ~2-2.5 hours total\n"
    ...
    # 2. Phase4
    ok, elapsed = run_script("phase4_backtester.py", "PHASE4 V1.0")
